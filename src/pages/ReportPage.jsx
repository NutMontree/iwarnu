...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://iwarnu-server.vercel.app/reports/${id}`)
      .then((response) => {
  ...
  return (
    <div className="mt-4 rounded-3xl bg-gray-100 mx-8 px-8 pt-8 pd-4">
      ...
      <div className="font-semibold text-xm mt-2">
        {" "}
        สถานที่ที่เกิดเหตุ
      ...
      <div className="mt-6 mb-8  md:grid-cols-[2fr_1fr]">
        <div className="my-4">
          <h2 className="font-semibold text-xm">
            เบอร์โทรศัพท์ : {report.phone}
          </h2>
          <h2 className="font-semibold text-xm">
            วันที่เกิดเหตุ : {report.date}
          </h2>
          <h2 className="font-semibold text-xm">
            เวลาที่เกิดเหตุ : {report.time}
          </h2>
        ...
        >
          <button
            className="mt-4 bg-red-500 text-white rounded-3xl p-2 px-6 "
